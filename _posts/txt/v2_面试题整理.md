...
15. classloader类加载机制，双亲委派模型
    ...
    1. Java的ClassLoader有三个，双亲委派机制，先调用父类的，父类加载失败再调用ExtClassLoader，都失败就调用BootstrapClassLoader。
    2. android的ClassLoader和JVM有点区别：PathClassLoad和DexClassLoader（加载系统的，加载任意位置的）—热修复
    3. 这里提一嘴热更新，自定义ClassLoader，重写findClass，loadClass是改变双亲委派机制的不要重写。字节流转换为Object对象
...
17. 单例和静态变量，静态方法的区别
    ...
    1. 单例特点：使用更灵活，保证某类只存在唯一实例（可以避免多处赋值）；该类本身完成自身的初始化；获取该类实例的方式很明确；延时初始化（静态类第一次加载时），静态方法产生的对象用完会被释放（单例不会，静态的引用）
    2. 静态变量：任何位置可重赋值；不断初始化费资源（所以用静态类）
...
19. 单例模式，懒汉和饱汉写法 
    ...
    5. 单例模式：不建议第一种，第二种，建议第三种，只有在明确要使用lazy loading效果时才会使用第五种
       ...
       3. ```
          //饿汉式，线程安全，类加载时初始化浪费内存，基于ClassLoader机制避免多线程同步问题存
...
1. ThreadLocal 原理
   ...
   2. 也就是说ThreadLocal类型的本地变量是存放在具体的线程空间上，其本质相当于一个装载本地变量的工具壳，通过set方法将value添加到调用线程的threadlocals中，当调用线程调用get方法时能够从它的threadLocals中取出变量。本地变量不用了要remove，因为如果线程一直不消亡，可能会OOM
...
5. 怎么启动service，service和activity怎么进行数据交互
   1. 答：startService，bindService
      ...
      3. 都开启时：先stop无反应，调用unbindService时才能销毁Service；先unbindService只能onUnbind不能销毁，调用stopService时才可销毁
...
15. 序列化知道吗，说一下为什么要序列化
    ...
    3. serialVersionUID是用来辅助对象的序列化反序列化的，显示定义，JVM自动生成的比较敏感，容易失败
...
23. 事件分发机制
    ...
    2. 事件分发的核心就是dispatchTouchEvent，整个View之间的事件分发就是一个大的递归函数（dispatchTouchEvent）。在递归的过程中会适时调用onInterceptTouchEvent来判断是否拦截事件，或调用onTouchEvent来处理事件
...
3. handler原理，是如何实现延时的
   1. 答：
      ...
      2. looper.next拿消息时会判断当前时间和msg时间
...
9. Looper.loop() 为什么不会阻塞主线程
   ...
   1. next()中的nativePollOnce 方法是一个 native 方法，当调用此 native 方法时，主线程会释放 CPU 资源进入休眠状态，直到下条消息到达或者有事务发生，通过往 pipe 管道写端写入数据来唤醒主线程工作，这里采用的 epoll 机制
10. Handler延时不准是因为计算when时间时，没包括深度休眠时间
11. 为什么不会ANR，为什么不会阻塞主线程？：
    1. nativePollOnce这个native方法，阻塞唤醒机制，主线程会释放CPU资源进入休眠，等到下条消息或新事务到达头部时，唤醒（提一嘴epoll机制，观察者模式，往pipe管道写数据进行唤醒线程）
...
7. sleep不释放锁和wait释放锁，底层怎么做的
   ...
   2. 属于Thread类，让当前线程停止执行，阻塞，把CPU让给其他线程执行，但不会释放对象锁，到指定时间后线程又会自动恢复到可运行状态。底层native方法
...
12. synchronized和Lock的区别
    ...
    2. CAS：
       ...
       4. 缺点：反复循环CPU开销大；ABA问题（解决方案是加版本号-AtomicStampedReference类）
...
9. 背压Backpressure：目的是为了控制流量，（用zip组合来举例子）因为水缸存储的能力有限
   ...
   1. 背景：上下游工作在同一个线程中时，这时是一个同步的订阅关系，上游每发送一个事件必须等下游接收处理完，才能继续发，所以不会OOM；当在不同线程中时，是一个异步的订阅关系，这时上游发数据不需等下游接收，因为两个线程不能直接进行通信，这时就需要帮助，也就是我们刚说的`水缸` 上游发到水缸里，下游从水缸里取事件来处理，因此上游发的过快下游处理速度太慢，水缸就会慢溢出来，就会OOM。同步异步的区别仅在于是否有水缸！！！
   ...
   2. 解决方法：
      ...
      1. 从源头想办法，不让你发的那么快
         ...
         1. 数量上考虑，但会丢失大部分事件
            ...
            2. sample采样操作符，每隔指定的时间就从上游取出一个事件发送给下游（放入水缸），设置时间
         2. 减慢发送事件的速度，上游通过适当的延时，不但减缓了事件进入水缸的速度，也让下游有充足时间从水缸去除数据处理，避免大量的事件涌进水缸

      2. Flowable
         1. Flowable代替Observable，创建时会选择参数，用来选择背压模式，也就是上下游流速不均衡时该怎么处理的办法（比如直接抛异常）
         ...
         4. 要上游正确的实现了如何根据下游的处理能力来发，否则也不对，缓存水缸大小是128，超过128个事件就会抛出异常，提示上游发的太多了，下游处理不过来，解决方法：
         ...
         5. 虽然是响应式拉取，但只是下游一个个处理，而上游是一次发送所有事件，排着队由下游处理。所以下游Subscription.request(n) 告诉上游，下游能处理多少，上游也要根据下游的能力去正确发送事件。
            1. 注意：理想很美好，但只有当上游正确的实现了 如何 根据下游处理能力来发送事件，才能达到这种效果！！！
            1. 同步情况下：上游是 `FlowableEmitter.requested()`  意思是当前外部请求的数量，这样来修改。也就是下游调用request(n)就会直接改变上游的requested的值，多次调用会叠加，而上游每发一个事件后就会减去，当减到0时，就代表下游没有处理能力了！！！，继续发就会异常了
            3. 异步：上下游在不同线程时，每个线程都有一个requested。发送事件前通过emitter.requested()==0来判断是否处理的过来。
