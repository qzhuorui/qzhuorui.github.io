...
15. classloader类加载机制，双亲委派模型
    ...
    1. Java的ClassLoader有三个，双亲委派机制，先调用父类的，父类加载失败再调用BootstrapClassLoader，都失败就调用ExtBotClassLoader。
    2. 这里提一嘴热更新，自定义ClassLoader，重写findClass，loadClass是改变双亲委派机制的不要重写。字节流转换为Object对象
...
18. 比较两个对象怎么比较，equals，== ；==和equals区别，重写equals怎么写,hashcode作用
    ...
    2. 比较时要重写equals方法，调的是Object的equals最终还是走到了==方法
    3. 基本数据类型和String在方法区的常量池中，相同的内容只会指向一个地址，所以==可以比较
    4. HashMap放入自定义Bean时，需要重写hashCode和equals，hasCode不同那就不必了铁定不同，相同会先用hashCode比较相同再用equals比较
    ...
    5. 单例模式：不建议第一种，第二种，建议第三种，只有在明确要使用lazy loading效果时才会使用第五种
       ...
       5. ```
          ...
          private static class QzrHolder{
              private static final Qzr instance = new Qzr();
          }
...
10. 热更新原理
   1. 自定义ClassLoader加载自定义位置下的字节码文件
...
23. 事件分发机制
        ...
        2. 事件分发的核心就是dispatchTouchEvent，整个View之间的事件分发就是一个大的递归函数，这个函数就是dispatchTouchEvent方法。在递归的过程中会适时调用onInterceptTouchEvent来拦截事件，或调用onTouchEvent来处理事件
        3. 宏观上整个dispatchTouchEvent源码是三部分：
           ...
           3. 根据mFirstTouchTarget，再次分发事件
              1. 根据 mFirstTouchTarget 重新分发事件
              2. 两种情况：
                 1. mFirstTouchTarget==null，说明上述事件分发中并没有子View对事件捕获，如果没有子 View 捕获处理 touch 事件，ViewGroup 会通过自身的 onTouchEvent 方法进行处理
                 2. mFirstTouchTarget 不为 null，说明在上面步骤 2 中有子 View 对 touch 事件进行了捕获，则直接将当前以及后续的事件交给 mFirstTouchTarget 指向的 View 进行处理
              3. 一旦子 View 捕获成功，后续的 MOVE 和 UP 事件是通过遍历 mFirstTouchTarget 链表，查找之前接受 ACTION_DOWN 的子 View，并将触摸事件分配给这些子 View。也就是说后续的 MOVE、UP 等事件的分发交给谁，取决于它们的起始事件 Down 是由谁捕获的。
           4. CANCEL事件：
